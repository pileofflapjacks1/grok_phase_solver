...
| C4 | If ≥ **~30%** of strong-\|E\| phases are correct within **~20°**, AI-PhaSeed extension can **strict-solve** those hard cells (oracle partial-φ curves). The ~20° window is non-centro TREF — fraction of strong \|E\| with \|Δφ\| ≤ 20°. Centrosymmetric PATT is a 0/π sign problem on \|F\|^{2}, not a 20° Patterson criterion | [`partial_seed_benchmark.md`](../data/processed/partial_seed_benchmark.md), [math](math/partial_seed.md) |
...
| C16 | On COD **2016452 / 2100301** experimental Fobs, **fragment_half** (SG-expanded ~½ ASU) mapCC **0.80 / 0.74** approaches or beats oracle **partial_30** (**0.72 / 0.71**); `auto` remains ~0.20. “Full $F_{\mathrm{calc}}$ soft prior” is fragment $F_{\mathrm{c}}$ phases on all $\mathbf{h}$ with a strong-\|E\| hard mask, not oracle $\varphi$ from the complete structure; $F_{\mathrm{o}}$ still constrain moduli only | [`cod_hard_path_validation.md`](../data/processed/cod_hard_path_validation.md), [math/partial_seed.md](math/partial_seed.md) |
