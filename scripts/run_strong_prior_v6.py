#!/usr/bin/env python3
"""
Train / evaluate GraphPhaseNet **v6** strong prior (GraPhAI HA-aware features).

v6 upgrades vs v5:
  - d_in=18 node features: v5 + ha_E_tail, low_res_w, E·low_res, κ_centrality
...
Writes:
  data/processed/strong_prior_v6.{npz,json,md}
...
def main():
    ...
    p = argparse.ArgumentParser(description="GraphPhaseNet v6 train + hold-out")
    p.add_argument("--quick", action="store_true")
    p.add_argument("--pilot", action="store_true")
    p.add_argument("--scale", action="store_true")
    p.add_argument("--scale-xl", action="store_true")
    p.add_argument("--n-structures", type=int, default=None)
    p.add_argument("--out", type=str, default="data/processed/strong_prior_v6.npz")
    ...
    if args.scale_xl:
        ...
        cfg = dict(
            ...
            hidden=160, n_layers=4, max_refl=140, scale_tag="v6_pilot",
    ...
    print("=== GraphPhaseNet v6 (GraPhAI HA features) ===", flush=True)
    ...
    summary = {
        ...
        "note": (
            ...
            "Legacy plateau ~21–22% frac≤20°. v6 GraPhAI HA features; "
            "not a claim of general macromolecular solution."
        ),
    }
    meta["holdout_v6"] = summary
    ...
    md.write_text(
        "\n".join(
            [
                ...
                f"**Features:** v6 d_in=18 · GraPhAI HA cues · Melgalvis gen · κ-gated edges",
