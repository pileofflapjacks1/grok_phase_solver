#!/usr/bin/env python3
"""
Train / evaluate GraphPhaseNet **v10** strong prior (v0.12).

v10 upgrades vs v9:
  - d_in=30 node features: v9 multipath depth (hop3, multipath span, Wilson B, E-outlier)
  - Stronger multipath κ-gated edges (feature_version=9, κ power ≥1.65)
...
Writes:
  data/processed/strong_prior_v10.{npz,json,md}
...
def main():
    ...
    p = argparse.ArgumentParser(description="GraphPhaseNet v10 train + hold-out")
    p.add_argument("--quick", action="store_true")
    p.add_argument("--pilot", action="store_true")
    p.add_argument("--scale", action="store_true")
    p.add_argument("--scale-xl", action="store_true")
    p.add_argument("--n-structures", type=int, default=None)
    p.add_argument("--out", type=str, default="data/processed/strong_prior_v10.npz")
    ...
    print("=== GraphPhaseNet v10 (d_in=30 large-cell / HA multipath) ===", flush=True)
    ...
    summary = {
        ...
        "note": (
            ...
            "Legacy plateau ~21–22% frac≤20°. v10 d_in=30 + Melgalvis large/HA; "
            "not a claim of general macromolecular solution."
        ),
    }
    meta["holdout_v10"] = summary
    ...
    md.write_text(
        "\n".join(
            [
                ...
                f"**Features:** v10 d_in=30 · large-cell / HA multipath · Melgalvis gen",
