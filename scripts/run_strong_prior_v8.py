#!/usr/bin/env python3
"""
Train / evaluate GraphPhaseNet **v8** strong prior (GraPhAI HA-aware features).
...
Writes:
  data/processed/strong_prior_v8.{npz,json,md}
...
def holdout_panel(model, n_eval=12, seed=4242, max_refl=140, n_extend=10, n_iter=40):
    from grok_phase_solver.data.synthetic_melgalvis import generate_melgalvis_structure, ha_heavy_config, actas2026_config
    from grok_phase_solver.metrics.stratified_prior import max_Z_from_elements, is_ha_bearing, stratify_holdout_rows
    rows = []
    ...
    for i in range(n_eval):
        ...
        # v0.10: mix organic P1 and HA-heavy Melgalvis for GraPhAI-style stratification
        if rng.random() < 0.45:
            cfg = ha_heavy_config() if rng.random() < 0.6 else actas2026_config()
...
def main():
    ...
    p = argparse.ArgumentParser(description="GraphPhaseNet v8 train + hold-out")
    p.add_argument("--quick", action="store_true")
    p.add_argument("--pilot", action="store_true")
    p.add_argument("--scale", action="store_true")
    p.add_argument("--scale-xl", action="store_true")
    p.add_argument("--n-structures", type=int, default=None)
    p.add_argument("--out", type=str, default="data/processed/strong_prior_v8.npz")
    ...
    p.add_argument(
        ...
        type=str,
        default="ha",
        choices=["none", "cod", "hard", "acta2026", "ha"],
        help="Melgalvis curriculum: ha = GraPhAI Z≥19-friendly; wilson match on by default",
    )
    p.add_argument(
        "--low-res-frac",
        type=float,
    ...
    if args.scale_xl:
        ...
        cfg = dict(
            ...
            hidden=160, n_layers=4, max_refl=140, scale_tag="v8_pilot",
    ...
    print("=== GraphPhaseNet v8 (GraPhAI multipath + bin CE) ===", flush=True)
    ...
    meta["cli"] = cfg
    meta["feature_version"] = 7  # same architecture as v7; v8 = curriculum + stratified reporting
    meta["d_in"] = 22
    meta["release"] = "0.10.0"
    ...
    summary = {
        ...
        "note": (
            ...
            "Legacy plateau ~21–22% frac≤20°. v8 GraPhAI multipath + bin CE; "
            "not a claim of general macromolecular solution."
        ),
    }
    meta["holdout_v8"] = summary
    meta["holdout_rows"] = rows
    from grok_phase_solver.metrics.stratified_prior import stratify_holdout_rows, format_stratified_md
    ...
    md.write_text(
        "\n".join(
            [
                ...
                f"**Features:** v8 d_in=22 · GraPhAI multipath · Melgalvis gen · κ-gated edges",
                ...
                "- Cluster scale: "
                "`python scripts/run_strong_prior_v8.py --scale-xl --melgalvis-preset cod`",
