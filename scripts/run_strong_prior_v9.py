#!/usr/bin/env python3
"""
Train / evaluate GraphPhaseNet **v9** strong prior (v0.11).

v9 upgrades vs v8:
  - d_in=26 node features: v7 multipath + log Vol, shell E std, κ×E, low-res rank
  - Stronger multipath κ-gated edges (feature_version=8)
  - Melgalvis large-cell / HA curricula (presets: ha, large, acta2026, cod, hard)
  - Hold-out mixes larger Vol + Z≥19 panels; stratified by HA/Z and Vol band
...
Writes:
  data/processed/strong_prior_v9.{npz,json,md}
...
def holdout_panel(model, n_eval=12, seed=4242, max_refl=140, n_extend=10, n_iter=40):
    ...
    for i in range(n_eval):
        ...
        # v0.11: emphasize large-cell + HA for GraPhAI-style stratification
        u = rng.random()
        if u < 0.35:
...
def main():
    ...
    p = argparse.ArgumentParser(description="GraphPhaseNet v9 train + hold-out")
    p.add_argument("--quick", action="store_true")
    p.add_argument("--pilot", action="store_true")
    p.add_argument("--scale", action="store_true")
    p.add_argument("--scale-xl", action="store_true")
    p.add_argument("--n-structures", type=int, default=None)
    p.add_argument("--out", type=str, default="data/processed/strong_prior_v9.npz")
    ...
    p.add_argument(
        ...
        type=str,
        default="large",
        choices=["none", "cod", "hard", "acta2026", "ha", "large"],
    ...
    print("=== GraphPhaseNet v9 (d_in=26 large-cell / HA multipath) ===", flush=True)
    ...
    summary = {
        ...
        "note": (
            ...
            "Legacy plateau ~21–22% frac≤20°. v9 d_in=26 + Melgalvis large/HA; "
            "not a claim of general macromolecular solution."
        ),
    }
    meta["holdout_v9"] = summary
    ...
    md.write_text(
        "\n".join(
            [
                ...
                f"**Features:** v9 d_in=26 · large-cell / HA multipath · Melgalvis gen",
