#!/usr/bin/env python3
"""
Train / evaluate GraphPhaseNet **v5** strong prior (Melgalvis-inspired features).

v5 upgrades vs v4:
  - d_in=14 diffraction-graph features (shell rank, E·deg, local E, shell |F|)
  - κ-gated adjacency reweight
  - Melgalvis generator + Wilson match curriculum defaults
  - Hard-region oversample + strong-|E| seed loss

Scale targets (wall-clock dependent):
  --quick     N≈80   (smoke)
  --pilot     N≈400  (default here — honest pilot numbers)
  --scale     N≈2000
  --scale-xl  N≈5000–10000 (cluster-scale; long)

Writes:
  data/processed/strong_prior_v5.{npz,json,md}
  (and optional --out path)
...
def main():
    ...
    p = argparse.ArgumentParser(description="GraphPhaseNet v5 train + hold-out")
    p.add_argument("--quick", action="store_true")
    p.add_argument("--pilot", action="store_true", default=True)
    p.add_argument("--scale", action="store_true")
    p.add_argument("--scale-xl", action="store_true")
    p.add_argument("--n-structures", type=int, default=None)
    p.add_argument("--out", type=str, default="data/processed/strong_prior_v5.npz")
    ...
    if args.scale_xl:
        cfg = dict(
            ...
            hidden=192, n_layers=4, max_refl=160, scale_tag="v5_xl",
            hard_oversample=1.5, n_eval=16,
        )
    elif args.scale:
        cfg = dict(
            n_structures=2000, epochs_per=14, epochs_refine=6, n_global_passes=3,
            hidden=160, n_layers=4, max_refl=140, scale_tag="v5_scale",
            hard_oversample=1.4, n_eval=14,
        )
    elif args.quick:
        cfg = dict(
            n_structures=60, epochs_per=8, epochs_refine=4, n_global_passes=1,
            hidden=96, n_layers=3, max_refl=100, scale_tag="v5_quick",
            hard_oversample=1.2, n_eval=6,
        )
    else:
        # pilot default — balanced for laptop runs (~tens of minutes)
        cfg = dict(
            n_structures=400, epochs_per=12, epochs_refine=6, n_global_passes=2,
            hidden=160, n_layers=4, max_refl=140, scale_tag="v5_pilot",
    ...
    if args.continue_from:
        ...
        print(f"Continue from {cp}")

    print("=== GraphPhaseNet v5 ===")
    print(json.dumps({k: cfg[k] for k in cfg}, indent=2))
    ...
    summary = {
        ...
        "note": (
            ...
            "Legacy v3/v4 plateau ~21–22% frac≤20°."
        ),
    }
    meta["holdout_v5"] = summary
    ...
    js.write_text(json.dumps({"meta": meta, "summary": summary, "rows": rows}, indent=2, default=str))
    ...
    md.write_text(
        "\n".join(
            [
                ...
                f"**Features:** v5 d_in=14 · Melgalvis gen · Wilson match · κ-gated edges",
                "",
