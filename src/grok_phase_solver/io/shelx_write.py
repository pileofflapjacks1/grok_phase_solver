...
def write_hkl_shelx(
    ...
    """
    Write free-format SHELX HKL (HKLF 4 style): h k l I σI [batch], terminated by 0 0 0.

    If only amplitudes are given, I = F² and σI ≈ 0.1·I (synthetic-friendly default).
    """
    path = Path(path)
    hkl = np.asarray(hkl, dtype=np.int32).reshape(-1, 3)
    if intensities is not None:
        I = np.asarray(intensities, dtype=np.float64).reshape(-1)
    ...
    if sig_i is None:
        sig = np.maximum(0.1 * I, 1e-3)
    else:
        sig = np.asarray(sig_i, dtype=np.float64).reshape(-1)
    lines = []
    for i in range(len(hkl)):
        h, k, l = map(int, hkl[i])
        lines.append(f"{h:4d}{k:4d}{l:4d}{I[i]:12.4f}{sig[i]:12.4f}{batch:4d}")
