"""
PhAI neural network architecture (vendored, attribution below).

Source: public PhAI test code
  https://github.com/AndersOMadsen/PhAI/blob/main/PhAI_files_public/PhAI_jupyter_test.py
Authors: Larsen, Rekis, Madsen (Science 2024).

Only depends on torch + einops (no pandas/xraydb).
"""
    ...
    def __init__(
        ...
        *,
        ...
        self.net_convolution_layers = nn.Sequential(
            *[
                nn.Sequential(ConvolutionalBlock(filters, kernel_size=kernel_size, padding=padding))
...
def randomize_output(output: torch.Tensor) -> torch.Tensor:
    """Random 0 / -180° phase grid (centrosymmetric bins)."""
    for j in range(output.shape[1]):
        for jj in range(output.shape[2]):
            for jjj in range(output.shape[3]):
                output[0, j, jj, jjj] = -180.0 if random.randint(0, 1) == 1 else 0.0
    return output


def phases_from_logits(output_phases: torch.Tensor) -> torch.Tensor:
    """
    Convert network logits to phase degrees in {0, -180}.

    Matches upstream PhAI_jupyter_test.phases.
    """
    bin_size = 180.0
    offset = bin_size / 2
